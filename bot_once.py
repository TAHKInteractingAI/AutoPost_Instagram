...
def _write_pw_worker():
    ...
    lines = [
        ...
        '        # Instagram đôi khi hiện popup "Tiếp tục với tài khoản X?" trước khi vào home\n',
        '        for _acc_try in range(3):\n',
        '            try:\n',
        '                acc_clicked = page.evaluate("""\n',
        '                    () => {\n',
        '                        const keywords = ["tiếp tục", "continue"];\n',
        '                        const els = Array.from(document.querySelectorAll(\n',
        '                            "button,[role=\'button\'],[aria-label=\'Tiếp tục\'],[aria-label=\'Continue\'],div"\n',
        '                        ));\n',
        '                        const t = els.find(e => {\n',
        '                            if (e.offsetParent === null) return false;\n',
        '                            const txt = (e.innerText || e.textContent || "").trim().toLowerCase();\n',
        '                            const aria = (e.getAttribute("aria-label") || "").trim().toLowerCase();\n',
        '                            return keywords.some(k => txt === k || aria === k);\n',
        '                        });\n',
        ...
        '                """)\n',
        '                if acc_clicked:\n',
        '                    log(f"   🖱️ Click account selector: {acc_clicked}")\n',
        '                    time.sleep(random.uniform(3.0, 5.0))\n',
        '                    log(f"   🔗 URL sau account selector: {page.url}")\n',
        '                    # Lưu session sau khi qua account selector\n',
        '                    context.storage_state(path=SESSION_FILE)\n',
        '                    with open(SESSION_FILE, "rb") as _sf:\n',
        '                        _b64 = __import__("base64").b64encode(_sf.read()).decode()\n',
        '                    log(f"NEW_SESSION_B64:{_b64}")\n',
        '                    break\n',
        '                else:\n',
        '                    break  # Không có account selector, tiếp tục bình thường\n',
        '            except Exception as _ae:\n',
        '                log(f"   ⚠️ Account selector lỗi: {_ae}")\n',
        '                break\n',
        '\n',
        '        dismiss_popups(page)\n',
